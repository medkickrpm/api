...
func ProcessCPTCode99453() error {
	...
	db := database.DB.Model(&models.PatientService{}).
		...
		Joins("LEFT JOIN last_bill_entries ON last_bill_entries.patient_id = patient_services.patient_id").Where("last_bill_entries.c99453 = 0 OR last_bill_entries.c99453 IS NULL")
